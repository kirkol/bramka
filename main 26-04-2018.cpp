...
int main(){
    ...
    wait(3); // potrzebuje tego czasu, bo jak trafia sie dwa resety pod rzad, to potrafi zablokowac przy inicjalizacji
    ...
    if (ntp.setTime("0.pl.pool.ntp.org") == 0){
        ...
        pc.printf("%s\r\n",buffer);
        pc.printf("%d", ctTime);
        if (ctTime<946681200 || ctTime == -1){ // jezeli czas jest mniejszy niz rok 2000 (czyli ustawilo czas np. na 1970 rok - czas domyslny. Ustawia sie, gdy sterownik nie mogl nawiazac polaczenia z NTP)
            pc.printf("Error\r\n");
            WrongTimeError();
        }
    }
...
//
    ...
    while(1){
        ...
        // mrugniecie diody led3 -> pokazanie czy czy cykl sie wykonuje
        led3=1;
        wait(aproxCycleTime);
        led3=0;
        ...
        if(strstr(hour, "18:30:0") != NULL){ // sterownik ma ustawiony czas z NTP, czyli ZAWSZE czas UTC (-1h lub -2h w strosunku do czasu polskiego)
        ...
        if(internetA == 0){
            pc.printf("NET OFF\r\n");
            led1 = 1; wait(0.1); led1 = 0; // miganie led1 sygnalizuje brak internetu
        }
        ...
        sockConnB = sockConnA;
        if(sockConnA != 0){
            pc.printf("SERV_STATE=%d\r\n", sockConnA); // jezeli cos nie tak z serwerem, to printuj nr bledu
            led4 = 1; wait(0.1); led4 = 0; // miganie led4 sygnalizuje blad polaczenia z serwerem
        }else{
            pc.printf("OK");
        }
        i++;
