...
int ID12RFID::read()
{
    ...
    if(_rfid.readable()){
        while (_rfid.getc()!=("%d",2)){ // jesli kod zacznie sie od znaku innego niz 2, to zwroci v=123 (dostep zostanie odrzucony - trzeba jeszcze raz przylozyc karte)
            v = 123;
            pc.printf("TU SIE PSUJE");
    ...
    for (int i=9; i>=0; i--) {                    // czytanie 10 pierwszych bajtow
        if(_rfid.readable()){
            pc.printf(" BAJT:"); 
            char c = _rfid.getc();                    // czytaj bajt jako char
            pc.printf("%d",c);
            ...
            int part = c - '0';                       // odejmij '0' (czyli 48) - z ASCII "przejscie na decymalny" (widzimy cyfry jakie odpowiadaja ASCII table)
...
int SC16IS750::read_from() // funkcja analogiczna do read() tylko dla WYJSCIA 2 (to sztucznie zrobiony UART)
{
    ...
    while (serial_bridge.getc() !=("%d",2)){ // jesli kod zacznie sie od znaku innego niz 2, to zwroci v=123 (dostep zostanie odrzucony - trzeba jeszcze raz przylozyc karte)
        v = 123;
    ...
    while(serial_bridge.getc()!=("%d",3)&&serial_bridge.getc()!=("%d",-1)){   // czytaj do czasu az trafisz na 3 (oznacza, ze tekst skonczyl sie prawidlowo)                  
        pc.printf("%d",serial_bridge.getc()); // lub az trafisz na -1 (oznacza, ze tekst zakonczyl sie zbyt szybko i odczytany zostal fragment danych, a nastepnie puste znaki)
    }
...
void WriteToSD(int id_card_number)
{
    ...
    ostringstream ss;
    ss << id_card_number;
    string NrKartyString = ss.str();
    
    if(NrKartyString.length()==1){
        NrKartyString.append("000000000");
    }else if(NrKartyString.length()==2){
        NrKartyString.append("00000000");
    }else if(NrKartyString.length()==3){
        NrKartyString.append("0000000");
    }else if(NrKartyString.length()==4){
        NrKartyString.append("000000");
    }else if(NrKartyString.length()==5){
        NrKartyString.append("00000");
    }else if(NrKartyString.length()==6){
        NrKartyString.append("0000");
    }else if(NrKartyString.length()==7){
        NrKartyString.append("000");
    }else if(NrKartyString.length()==8){
        NrKartyString.append("00");
    }else if(NrKartyString.length()==9){
        NrKartyString.append("0");
    }
    
    fprintf(fp,"%s \r\n",NrKartyString);       // sprawdz czy nie trzeba pisac znaku nowej linii
...
{    
    ...
    char buf[11];
    sprintf(temp_buf, "%s", NrKartyString);    // konwersja int na tablice char
    ...
    while (!feof(fp)) {                         // dopoki nie ma konca pliku
        fgets(buf,11,fp);                        // pobieranie 9 znakow do bufora buf
...
void send_to_SD(int _id,string action, string number)
{
    ...
    if ((czasPrzejscia>1490486400&&czasPrzejscia<1509235200)||(czasPrzejscia>1521936000&&czasPrzejscia<1540684800)||(czasPrzejscia>1553990400&&czasPrzejscia<1572134400)||
        ...
        czasPrzejscia = time(NULL)+7200; // dodanie 2 godzin do czasu z serwera
    }else{
        czasPrzejscia = time(NULL)+3600; // dodanie 1 godziny do czasu z serwera
    }
    strftime(buf,40,"%Y-%m-%d %H:%M:%S",localtime(&czasPrzejscia)); // ustawienie formatu daty
    
    //string linia = id_to_string+"$"+action+"$"+buf+"$"; // ta linia zostanie zapisana do pliku buffor.txt
...
void send_to_DB()
{
    ...
    while (!feof(fp)) {                             //czytaj dopoki nie ma konca pliku
        ...
        x=fgetc(fp); // czyta decymalne "13" (carriege return)
        pc.printf("%d", x);
        x=fgetc(fp); // czyta decymalne "10" (line feed)
        pc.printf("%d", x);
        
        //send line to DB via server
        ...
        if(buf4idstring!=""){
            ...
            if(conn==0){
                ...
                pc.printf(" nie ma polaczenia z socketem ");
            }
            sock.close();
        }
    }
    ...
    //jesli jest polaczenie z siecia i z socketem, to nadpisz plik buffor plikiem pustym
    
    if(ether.link()==true&&conn==0){
        pc.printf(" NADPISUJE PLIK PUSTYM PLIKIEM ");
